...
sp.restart.alg <- function(suffstat, alpha, data.dag){
	...
	start.order <- lapply(1:10, function(x) mindeg(g.undir))
...
t1 <- Sys.time()
grspdag <- sp.restart.alg(suffstat, 0.01, dag.list[[i]])
t2 <- Sys.time()
print(sum(grspdag))
graph <- grspdag
graph <- graph | t(graph)
...
suffstat <- list(C=cor(data.list[[i]]), n=nrow(data.list[[i]]))
t1 <- Sys.time()
pcdag <- pc.alg(suffstat, 0.01)
t2 <- Sys.time()
graph <- as(pcdag, "matrix")
graph <- graph | t(graph)
print(sum(graph) / 2)
print(t2 - t1)

t1 <- Sys.time()
score <- new("GaussL0penObsScore", data=data.list[[i]], intercept = FALSE, use.cpp = TRUE)
#estimate essential graph
ges.fit <- ges(score)
t2 <- Sys.time()
graph <- as(ges.fit$essgraph, "matrix")
graph <- graph | t(graph)
print(sum(graph) / 2)
print(t2 - t1)

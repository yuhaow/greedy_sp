...
grspdag.list <- lapply(1:dagnum, function(i) {
	...
	grspdag <- sp.restart.alg(suffstat, alpha, dag.list[[i]])
	dag2cpdag(as(grspdag, "graphNEL"))
})
save(grspdag.list, dag.list, file=paste("result/", strsplit(basename(args[4]), ".", fixed=TRUE)[[1]][1], "_alpha_", toString(alpha), ".rda", sep=""))

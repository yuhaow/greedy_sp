...
sp.restart.alg <- function(param){
	#get the input result
	suffstat <- param$suffstat
	data.dag <- param$dag
...
param.list <- lapply(1:dagnum, function(i) list(suffstat=list(C=cor(data.list[[i]]), n=nrow(data.list[[i]])), dag=dag.list[[i]]))
grspdag.list <- parLapply(cl, param.list, sp.restart.alg)

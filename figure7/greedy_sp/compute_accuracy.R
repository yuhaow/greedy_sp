...
for(i in 1:8)
	print(true_descendents[which(gene_names == perturbations[i]), i])

for(j in 1:8){
	print("perturbations")
	print(perturbations[j])
	for(i in 1:nrow(true_descendents))
		if(true_descendents[i,j] > 3){
			print(gene_names[i])
		}
}
...
slope = sapply(1:50 / 10, function(t) {(sum(true_descendents > t) - 8) / sum(true_descendents <= t)})
#slope = sapply(1:50 / 10, function(t) {sum(true_descendents > t) / sum(true_descendents >= 0)})

results = data.frame(1, true_positives, false_positives)
save(results, slope, file=paste('./accuracy_results/', paste(sig,'accuracy_results.rda', sep = '_'), sep = ''))

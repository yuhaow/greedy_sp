...
load('greedy_sp/greedy_sp_summary.rda')
third_batch = do.call(cbind, list(false_positives[idx,], true_positives[idx,]))
colnames(third_batch) = c('fp', 'tp')

load('pc/greedy_sp_summary.rda')
pc_batch = do.call(cbind, list(false_positives[idx,], true_positives[idx,]))
colnames(pc_batch) = c('fp', 'tp')
pc_batch = data.frame(pc_batch)

load('ges/greedy_sp_summary.rda')
ges_batch = do.call(cbind, list(false_positives[idx,], true_positives[idx,]))
colnames(ges_batch) = c('fp', 'tp')
ges_batch = data.frame(ges_batch)
...
tpplot <- function(gauss.plot, pc.plot, ges.plot, title, alphas){
  print(pc.plot$tp)
  gauss.plot = data.frame(gauss.plot)
  #gies.plot = data.frame(gies.plot)
  #maxy = max(c(gauss.plot$tp, gies.plot$tp))
  #maxx = max(c(gauss.plot$fp, gies.plot$fp))
	print(length(ges.plot$tp))
  maxy = max(c(gauss.plot$tp, pc.plot$tp, ges.plot$tp))
  maxx = max(c(gauss.plot$fp, pc.plot$fp, ges.plot$fp))
  png(paste(title, ".png", sep=""))
  par(mar=c(5,5.5,3,1))
  plot(x=gauss.plot$fp, y=gauss.plot$tp, xlim=c(0, maxx), ylim=c(0, maxy), xlab="number of false positives", ylab="number of true positives", col="blue", type="p", pch="O", axes=FALSE, cex.lab=2)
  plot(x=gauss.plot$fp, y=gauss.plot$tp, xlim=c(0, maxx), ylim=c(0, maxy), axes=FALSE,  xlab="number of false positives", ylab="number of true positives", 
  ...
  print(gauss.plot$fp)
  print(gauss.plot$tp)
  lines(x=pc.plot$fp, y=pc.plot$tp, col="red", type="p", pch=20)
  text(pc.plot$fp, pc.plot$tp, labels=sapply(alphas, toString), cex=0.7, pos=3)
  lines(x=ges.plot$fp, y=ges.plot$tp, col="purple", type="p", pch=3)
  print(ges.plot$fp)
  abline(0,slope[idx])
  print(alphas)
  legend("bottomright", legend=c("GSP", "GES", "PC"), col=c("blue", "purple", "red"), pch=c(1, 3, 20), cex=2)
  dev.off()
}

tpplot(third_batch, pc_batch, ges_batch, 'plot', row.names(third_batch))

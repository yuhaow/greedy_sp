...
simulation <- function(dag.list, lambda){
	...
	dag.alg <- function(dag){
		...
		grspdag.list <- lapply(c(1, 5, 10), function(ordernum) lapply(c(4), function(depth) sp.restart.alg(dag$dagmat, lambda, ordernum, depth)))
...
save(curve, file=paste("result/lambda_", toString(lambda), "_", suffix, "_sp_d4.rda", sep=""))

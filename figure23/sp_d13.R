...
sp.restart.alg <- function(mat, lambda, ordernum, depth){
	...
	get.newdag <- function(dag, order, edge, vorders){
		...
		if(length(par) != 0){
			dag[par, edge[1]] <- sapply(1:length(par), function(i) abs(pcorOrder(par[i], edge[1], c(par[-i], edge[2]), cormat)) > lambda)
			dag[par, edge[2]] <- sapply(1:length(par), function(i) abs(pcorOrder(par[i], edge[2], par[-i], cormat)) > lambda)
		}
	...
	sing.restart <- function(order){
		...
		while(TRUE){
			...
			if((length(rdags) > 0 && length(vtrace) != depth) || length(select) != 0){
				if(length(select) != 0){
					vorders <- list()
					vtrace <- list()
					vdags <- list()
...
simulation <- function(dag.list, lambda){
	...
	dag.alg <- function(dag){
		...
		grspdag.list <- lapply(c(1, 5, 10), function(ordernum) lapply(c(1, 3), function(depth) sp.restart.alg(dag$dagmat, lambda, ordernum, depth)))
	...
	faithprop <- rowSums(result[1:6,]) / dagnum
	aveshd <- rowSums(result[7:12,]) / dagnum
...
save(curve, file=paste("result/lambda_", toString(lambda), "_", suffix, "_sp.rda", sep=""))
